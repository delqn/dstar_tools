...
class Parser(object):
    @classmethod
    def _parse_drats_message(cls, packet):
        msg = {'raw_data': packet}
    ...
    @classmethod
    def _is_valid_drats(cls, buff):
        return (buff and len(buff) > drats_head_len + drats_tail_len and
                DRATS_HEAD in buff and
                DRATS_TAIL in buff[buff.index(DRATS_HEAD):])

    @classmethod
    def search_drats_message(cls, buff):
        if cls._is_valid_drats(buff):
            begin = buff.index(DRATS_HEAD) + drats_head_len
            end = begin + buff[begin:].index(DRATS_TAIL)
            message = cls._parse_drats_message(buff[begin:end])
            return (message, buff[:begin - drats_head_len] + buff[end + drats_tail_len:])
        return (None, buff)

    @classmethod
    def _is_valid_dprs(cls, buff):
        return (buff and len(buff) > dprs_head_len + dprs_tail_len and
                DPRS_HEAD in buff and
                DPRS_TAIL in buff[buff.index(DPRS_HEAD):])

    @classmethod
    def search_dprs_message(cls, buff):
        if cls._is_valid_dprs(buff):
